...
export class UsersService {
  ...
  async createUserLoginGoogleAccount(
    ...
    if (getUserByEmail) {
      ...
      if (getUserByEmail.status === UserStatus.New) {
        try {
          await this.postgresService.executeInTransaction([
            ...
            WHERE id = $3 AND name IS NULL
            `,
              params: [UserStatus.Active, name, getUserByEmail.id],
            },
          ]);

          this.logger.log(`Cập nhật trạng thái New -> Active thành công, trường hợp tồn tại user trạng thái New khi đăng nhập bằng SSO GG ${email}`);
          return {
      ...
      if (getUserByEmail.status === UserStatus.Inactive) {
        throw new UnauthorizedException('Tài khoản Email của bạn đã bị khóa tại hệ thống của chúng tôi. Vui lòng liên hệ Admin để được hỗ trợ.');
      }
    ...
    try {
      const result = await this.postgresService.execute<{ email: string, name: string }>(
        ...
        `,
        [idUserNew, email, name, UserStatus.Active],
      );
  ...
  async getUserByEmail(email: string) {
    try {
      ...
      this.logger.error(
        `FKhông thể lấy được người dùng theo by email ${email}`,
        error.stack,
      );
      throw error;
    }
  ...
  async getUserById(id: string) {
    try {
      ...
      this.logger.error(
        `Không thể lấy được người dùng theo id: ${id}`,
        error.stack,
      );
      throw error;
    }
  ...
  async registerUser(
    ...
    const getUserByEmail = await this.getUserByEmail(email)
    if (getUserByEmail) {
      // 2.1 TH tồn tại user ở trạng thái New
      if (getUserByEmail.status == UserStatus.New) {
        return {
          status: 1,
          message: 'Đăng ký thành công. Vui lòng kiểm tra email của bạn để xác minh. Nếu trong Hộp thư đến của bạn không thấy có, vui lòng kiểm tra ở mục Thư rác',
        };
      }
      // 2.2 TH tồn tại user ở trạng thái Active
      if (getUserByEmail.status == UserStatus.Active) {
        return {
          status: 0,
          message: 'Email đã được đăng ký. Vui lòng hãy đăng ký địa chỉ khác hoặc ấn Quên mật khẩu để cài đặt lại mật khẩu cho Email của bạn'
        }
      }
    }
      ...
      await this.postgresService.executeInTransaction([
        // 3.1. Tạo user
        {
          sql: `
            ...
            RETURNING id
          `,
          params: [idUserNew, email, hashedPassword, UserStatus.New],
        },
      ]);
      const resulTokenEmail = this.generateTokenEmail(idUserNew, email)

      // 3.2. Gửi email xác thực
  ...
  async verifyEmailToken(token: string): Promise<{ status: number; message: string }> {
    try {
      ...
      if (!user || user.length === 0) {
        return {
          status: 0,
          message: 'Tài khoản không tồn tại trong hệ thống. Vui lòng đăng ký lại.'
        }
      }
      ...
      if (user.status === UserStatus.Active) {
        return {
          status: 0,
          message: 'Tài khoản này đã được kích hoạt trước đó. Vui lòng thực hiện đăng nhập dựa trên các thông tin mà bạn đã đăng ký.'
        }
      }
      ...
      try {
        ...
        return {
          status: 1,
          message: 'Tài khoản của bạn đã được kích hoạt thành công. Vui lòng thực hiện đăng nhập dựa trên các thông tin mà bạn đã đăng ký.'
        }
      } catch (error) {
        this.logger.error(
          "Cập nhật trạng thái New -> Active không thành công, trường hợp xác thực email",
          error.stack,
        );
        throw error;
      }
